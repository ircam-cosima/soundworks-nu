...
export default class NuRoomReverb {
  constructor(soundworksClient) {
    ...
    this.irMap = new Map();
    this.params = { audioFileId: 0, perc: 1, loop: true, accSlope: 0, timeBound: 0 };
  ...
  emitAtPos(irId, syncStartTime) {
    ...
    this.soundworksClient.status += 1; // ad + 1 to status (for one more source playing)
    ...
    gain.gain.value = normFactor * this.soundworksClient.propagParams.masterGain;
    ...
    if (syncStartTime > now) {
      var audioContextStartTime = audioContext.currentTime + syncStartTime - now;
    ...
    gain.connect(this.soundworksClient.audioAnalyser.in);
    requestAnimationFrame(this.soundworksClient.updateBkgColor);

    // timeout callback, runs when we finished playing
    setTimeout(() => {
      this.soundworksClient.status -= 1;
      if (this.soundworksClient.status == 0) { this.soundworksClient.renderer.setBkgColor([0, 0, 0]); }
